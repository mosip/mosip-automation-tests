...
public class BookSpecificSlotForPrid extends BaseTestCaseUtil implements StepInterface {
	...
	public void run() throws RigInternalError {
		if (step.getParameters() == null || step.getParameters().isEmpty() ||step.getParameters().size()<4) {
			...
			registration_center_id =step.getParameters().get(2);
			time_slot_from =step.getParameters().get(3);
			time_slot_to =step.getParameters().get(4);
		}
	...
	private void bookSlotForPrid(String prid) throws RigInternalError {
		...
		jsonReq.put(E2EConstants.PRE_REGISTRATION_ID, pre_registration_id);
