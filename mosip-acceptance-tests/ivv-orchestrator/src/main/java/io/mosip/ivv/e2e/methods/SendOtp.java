	...
	public void run() throws RigInternalError {
		...
		if (step.getParameters().isEmpty()) {
			...
			for (String resDataPath : step.getScenario().getResidentTemplatePaths().keySet()) {
				packetUtility.requestOtp(resDataPath, step.getScenario().getCurrentStep(), emailOrPhone,step);
			}
			...
			if (isForChildPacket && !step.getScenario().getGeneratedResidentData().isEmpty())
				packetUtility.requestOtp(step.getScenario().getGeneratedResidentData().get(0), step.getScenario().getCurrentStep(), emailOrPhone,step);
		} else {
			...
			if (personaFilePath.startsWith("$$")) {
				...
				packetUtility.requestOtp(personaFilePath, step.getScenario().getCurrentStep(), emailOrPhone,step);
			}
		}
	}
