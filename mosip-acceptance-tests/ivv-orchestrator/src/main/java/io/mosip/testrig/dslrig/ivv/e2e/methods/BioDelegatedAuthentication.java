...
public class BioDelegatedAuthentication extends BaseTestCaseUtil implements StepInterface {
	public static Logger logger = Logger.getLogger(BioAuthentication.class);
	private static final String BIOMETRIC_FACE = "idaData/BioAuthDelegated/BioAuthDelegated.yml";
	...
	public void run() throws RigInternalError {
		...
		if (step.getParameters().size() == 5) {
			if (step.getParameters().get(4).startsWith("$$")) {
				clientId = (String) step.getScenario().getOidcClientProp().get("clientId");
			} else {
				clientId = step.getParameters().get(4);
			}
		}
					...
					for (Object object : casesListVID) {
						...
						if (clientId != null) {
							if (test.getInput() != null) {
								test.setInput(test.getInput().replace("$clientId$", clientId));
							}
							if (test.getEndPoint() != null) {
								test.setEndPoint(test.getEndPoint().replace("$clientId$", clientId));
							}
